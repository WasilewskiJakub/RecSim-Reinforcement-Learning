...
class MusicListenerState(user.AbstractUserState):
    ...
    def score_document(self, doc_obs):
        ...
        return likes + doc_obs['production_value'] if favourite_genre == doc_obs['cluster_id'] else doc_obs['production_value']
    ...
    def observation_space(cls):
        ...
        return spaces.Dict({
            ...
            ,'quality_tolerance': spaces.Box(low=0.0, high=120.0, shape=tuple(), dtype=np.float32)
        })
...
class MusicListenerSampler(user.AbstractUserSampler):
    ...
    def sample_user(self):
        ...
        preferred_tempo = self._rng.uniform(40, 200)
        sensitivity_mood = self._rng.uniform(0.5, 1.5)
        quality_tolerance = self._rng.uniform(0.5, 0.9)
        remaining_time = self._rng.uniform(60.0, 600.0)
        exploration_tendency = np.clip(self._rng.normal(loc=0.3, scale=0.15), 0.02, 0.80)
...
class MusicResponse(user.AbstractResponse):
    ...
    def response_space(cls):
        return spaces.Dict({
            ...
            ,'mood_intensity': spaces.Box(low=2.20, high=7.30, shape=tuple(), dtype=np.float32)
