...
class MusicListenerState(user.AbstractUserState):
    ...
    def observation_space(cls):
        ...
        return spaces.Dict({
            ...
            ,'preferred_tempo': spaces.Box(low=MusicDocument.MIN_TEMPO, high=MusicDocument.MAX_TEMPO, shape=tuple(), dtype=np.float32)
...
class MusicListenerSampler(user.AbstractUserSampler):
    ...
    def sample_user(self):
        ...
        preferred_tempo = self._rng.uniform(MusicDocument.MIN_TEMPO, MusicDocument.MAX_TEMPO)
        sensitivity_mood = self._rng.integers(0, 2, size = MusicDocument.MOOD_COUNT)
        quality_tolerance = self._rng.uniform(0.01, 0.3)
...
class MusicResponse(user.AbstractResponse):
    ...
    def response_space(cls):
        return spaces.Dict({
            ...
            ,'tempo': spaces.Box(low=MusicDocument.MIN_TEMPO, high=MusicDocument.MAX_TEMPO, shape=tuple(), dtype=np.float32)
...
class MusicListenerModel(user.AbstractUserModel):
    ...
    def simulate_response(self, documents):
        ...
        for i, response in enumerate(responses):
            ...
            response.mood_intensity = documents[i].mood_intensity
